...
```{r hist, fig.height=8, fig.width=8, echo=TRUE}
...
```{r plot_steps_interval, fig.width=8, fig.height=8, echo=TRUE}
...
```{r histogram_steps, fig.width=8, fig.height=8, echo=TRUE}
hist(fs$totalSteps, breaks=20, freq=TRUE, xlab="Steps", main="Average number of steps")
```
Do these values differ from the estimates from the first part of the assignment? What is the impact of imputing missing data on the estimates of the total daily number of steps?

It seems inputing missing data cause the mean total number of steps increased.
```{r mean_steps_after_fixing_missing_value, echo=TRUE}
fm <-mean(fs$totalSteps) # this returns 362668
fm

fmedianSteps <-median(fs$totalSteps) # returns 11458
fmedianSteps
```
...
```{r weekday_weekend_pattern, echo=FALSE, result=FALSE}
require(lubridate)
require(lattice)
data_wkdays <- fdata %>%
...
```{r activity_graph, fig.width=8, fig.height=8, echo=TRUE}
